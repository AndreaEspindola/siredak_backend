const pool = require('../database/connection_database');
...
async function getAllClients() {
    ...
    try {
        ...
        for(let i = 0; i < result.length; i++){
            let oppositedField = await pool.query(`
...
async function getAddress(id_client) {
    try {
        //Logs
        await pool.query(`INSERT INTO logs (id_client, id_user, id_derecho_arco, date_performed) 
        VALUES (${id_client}, 1010, 1, "${moment().format()}")`)

        const opposition = await pool.query(`SELECT * FROM disable_fields WHERE id_client = ${id_client} AND id_field = 9`)
        
        if(opposition[0]==undefined){
            const result = await pool.query(`SELECT * FROM address WHERE id_client = ${id_client}`)
            return result;
        } else {
...
async function getIdIdentification(id_client) {
    try {
        ...
        const opposition = await pool.query(`SELECT * FROM disable_fields WHERE id_client = ${id_client} AND id_field = 10`)

        if(opposition[0]==undefined){
            const result = await pool.query(`SELECT * FROM identifications WHERE id_client = ${id_client}`)
            return result;
        } else {
...
async function searchClients(search, type) {
    try {
        ...
        const result = await pool.query(`SELECT * FROM clients WHERE ${field} LIKE '%${search}%'`)
        ...
        for(object in data) {
            ...
            if(data[object]!="") { 
                 if(typeof data[object] == "number") {
                    await pool.query(`UPDATE clients SET ${object} = ${data[object]} WHERE id_clients = ${id_client}`)
                } else {
                    await pool.query(`UPDATE clients SET ${object} = "${data[object]}" WHERE id_clients = ${id_client}`)
                }
            }
...
async function addOpposition(id_client, data) {
    try {   
        ...
        await pool.query(`DELETE FROM disable_fields WHERE id_client = ${id_client}`)
        for(let i = 0; i < data.length; i++) {
            await pool.query(`INSERT INTO disable_fields (id_client, id_field) VALUES (${id_client}, ${data[i]})`)
        }
...
async function getOppositionIdClient(id_client) {
    try {  
        ...
        return await pool.query(`SELECT * FROM disable_fields WHERE id_client = ${id_client}`)
    } catch (error) {
        return error;
    }
}
...
async function deleteData(id_client) {
    try {
        ...
        `)
          await pool.query(`DELETE FROM address WHERE id_client = ${id_client}`)
          await pool.query(`DELETE FROM identifications WHERE id_client = ${id_client}`)
...
async function getHistorical(initial, final){
    try {
        const result = await pool.query(`SELECT * FROM logs WHERE date_performed BETWEEN ${initial} AND ${final}`)
        return result;
    } catch (err) {
        return err
    }
